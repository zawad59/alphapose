...
class Mscoco(CustomDataset):
    ...
    def get_gt_keypoints(self, img_id):
        """Retrieve ground truth keypoints for a given image ID."""
        for ann in self.annotations:  # Assuming `annotations` is a list of all annotations
            if ann['image_id'] == img_id:
                keypoints = np.array(ann['keypoints']).reshape(-1, 3)  # COCO format: (x, y, visibility)
                return keypoints
        return None  # Return None if no keypoints are found
