...
class Mscoco_det(data.Dataset):
    ...
    '''def __getitem__(self, index):
        ...
        # Ensure bbox has four elements: [x_min, y_min, x_max, y_max]
        x1, y1, w, h = det_res['bbox']
        bbox = [x1, y1, x1 + w, y1 + h]

        # Apply transformation to image and bbox
        inp, bbox = self.transformation.test_transform(image, bbox)

        # Generate dummy labels and masks if ground truth is unavailable
        num_joints = len(self.EVAL_JOINTS)
        labels = torch.zeros((num_joints, 2))  # Replace with actual labels if available
        label_masks = torch.ones((num_joints, 2))  # Masks set to 1 by default, update if needed

        # Debugging information for troubleshooting
        print(f"Index: {index}")
        print(f"Input type: {type(inp)}, Input dtype: {inp.dtype}")
        print(f"BBox type: {type(bbox)}, BBox: {bbox}")
        print(f"Labels shape: {labels.shape}, Label masks shape: {label_masks.shape}")

        return inp, torch.tensor(bbox, dtype=torch.float32), torch.tensor([det_res['bbox']], dtype=torch.float32), \
            torch.tensor([det_res['image_id']], dtype=torch.int32), torch.tensor([det_res['score']],
                                                                                 dtype=torch.float32), \
            torch.tensor([imght], dtype=torch.float32), torch.tensor([imgwidth], dtype=torch.float32), \
            labels, label_masks'''
    ...
    def __getitem__(self, index):
        ...
        # Return the required 7 elements
        return inp, torch.tensor(bbox, dtype=torch.float32), torch.tensor([det_res['bbox']], dtype=torch.float32), \
            torch.tensor([det_res['image_id']], dtype=torch.int32), torch.tensor([det_res['score']],
                                                                                 dtype=torch.float32), \
            torch.tensor([imght], dtype=torch.float32), torch.tensor([imgwidth], dtype=torch.float32)
