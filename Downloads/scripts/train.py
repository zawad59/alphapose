...
def main():
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (i + 1) % opt.snapshot == 0:
            # Save checkpoint
            torch.save(m.module.state_dict(), './exp/{}-{}/modelCheckPoint_{}.pth'.format(opt.exp_id, cfg.FILE_NAME, opt.epoch))
            # Prediction Test
            with torch.no_grad():
        ...
        if i == cfg.TRAIN.DPG_MILESTONE:
            torch.save(m.module.state_dict(), './exp/{}-{}/finalCheckPoint.pth'.format(opt.exp_id, cfg.FILE_NAME))
            ...
            train_loader = torch.utils.data.DataLoader(
                ...
                shuffle=True,
                num_workers=4  # Adjust to your system capacity
            )

    torch.save(m.module.state_dict(), './exp/{}-{}/final_DPGCheckPoint.pth'.format(opt.exp_id, cfg.FILE_NAME))
